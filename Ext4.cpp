...
int Ext4::isExt4(const char *fsPath) {
    ...
    if (strptr == NULL) {
        ...
        if (strptr == NULL) {
            SLOGI("Filesystem type of %s is not EXT4", fsPath);
        } else {
            rc = true;
        }
    } else {
...
int Ext4::check(const char *fsPath) {
    ...
    do {
        ...
        case 8:
            ...
            return -1;
        default:
            SLOGE("EXT4 Filesystem check failed (unknown exit code %d)", rc);
            errno = EIO;
            return -1;
        }
    } while (0);
...
int Ext4::doFuse(char *src, char *dst) {
    char *args[6];
    pid_t fusePid;
    ...
    if (fusePid == 0) {       
        SLOGW("Firing up storage daemon (%s) -> (%s)", src, dst);
