...
include $(CLEAR_VARS)
LOCAL_MODULE:= vold
LOCAL_SRC_FILES := $(common_src_files) $(extra_src_files) main.cpp
LOCAL_C_INCLUDES := $(common_c_includes)
LOCAL_CFLAGS := $(common_cflags)
LOCAL_SHARED_LIBRARIES := $(common_libraries) libcrypto
LOCAL_STATIC_LIBRARIES := $(common_static_libraries)
...
LOCAL_SRC_FILES := $(common_src_files) main.cpp
LOCAL_C_INCLUDES := $(common_c_includes)
LOCAL_CFLAGS := $(common_cflags) -DMINIVOLD
LOCAL_STATIC_LIBRARIES := $(common_libraries) $(common_static_libraries)
LOCAL_STATIC_LIBRARIES += libcrypto_static libc libstdc++ libext2_uuid
