...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (isUnixFs) {
            ...
            if (chown(fuseSrc, FUSE_SDCARD_UID, FUSE_SDCARD_GID)) {
                SLOGE("Failed to set owner/group on %s (%s)", fuseSrc, strerror(errno));
                goto fail_fuse_mount;
            }

            // Set permissions (775) on fuse dir
            if (chmod(fuseSrc, S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH)) {
...
int Volume::doFuseMount(char *src, char *dst) {
    ...
    args[1] = src;
    args[2] = dst;
    args[3] = (char*) "FUSE_SDCARD_UID";
    args[4] = (char*) "FUSE_SDCARD_GID";
    args[5] = NULL;
